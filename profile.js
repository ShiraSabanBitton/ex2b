...
app.get("/profile-data", (req, res) => {
  ...
  fs.readdir(profilePath, (err, files) => {
    ...
    const textFiles = files.filter(
      (file) => file.startsWith("text") && file.endsWith(".txt")
    );
    let contentFiles = textFiles.map((file) =>
      fs.readFileSync(path.join(profilePath, file), "utf8")
    );

    let bioText = fs.readFileSync(path.join(profilePath, "bio.txt"), "utf8");
    const htmlBioContent = bioText.split(":");

    let fullTitleText = fs.readFileSync(
      path.join(profilePath, "title.txt"),
      "utf8"
    );
    ...
    const profilesNames = filesInPri.filter((file) =>
      fs.statSync(path.join(privateFolder, file)).isDirectory()
    );
